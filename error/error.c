...
const char *token_name(TokenType token) {
    switch (token) {
        ...
        case TOK_PREPROCESSOR: return "Preprocessor Directive";
...
TokenType next_token() {
    ...
    // Skip whitespaces and handle newlines
    while (isspace(ch)) {
        if (ch == '\n') line++;
        ch = source[pos++];
    }
    ...
    if (isalpha(ch)) {
        ...
        // Check for keywords
        if (strcmp(lexeme, "int") == 0 || strcmp(lexeme, "return") == 0 || strcmp(lexeme, "printf") == 0) {
            return TOK_KEYWORD;
        }
...
int main(int argc, char *argv[]) {
    // Opening the file 'input.c' directly
